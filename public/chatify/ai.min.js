...
function promptPick(focusTab = "builtIn", type = PromptPickType.Default, mkMsg = true) {
  return new Promise((resolve, reject) => {
    ...
    function setTabContent(tab, prompts, savedPromptsShowDeleteButton = false) {
      ...
      controlsBox.appendMany(new Html("div").class("row", "py-0").appendMany(new Html("button").text("Create your own prompt").classOn("fg-auto").on("click", () => {
        setPrompt({ id: "custom", label: "Custom" }, mkMsg);
        modal4.hide();
        resolve("custom");
    ...
    const modalContent = new Html("div").class("fg").append(new Html("span").text("Prompt selection")).append(new Html("div").appendMany(tabsButtons).appendMany(tabsGroup));
...
async function request(text, addUserMessage = true) {
  if (store.get("mpState") === true) {
    ...
    for (let i = 0;i < finalResponse.length; i++) {
      ...
      const prompt = store.get("prompts").find((p) => p.displayName === currentPrompt);
      if (prompt === undefined) {
