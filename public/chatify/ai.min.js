...
function promptPick(focusTab = "builtIn", type = PromptPickType.Default, mkMsg = true) {
  return new Promise((resolve, reject) => {
    ...
    function setTabContent(tab, prompts, savedPromptsShowDeleteButton = false) {
      ...
      }), store.get("apiUsage").allowMultiPrompt === true ? type === PromptPickType.Default ? new Html("button").text(`Multi-prompt ${store.get("mpState") === true ? "(On)" : "(Off)"}`).append(new Html("span").class("badge").text("BETA")).classOn("transparent", "fg-auto").on("click", () => {
        modal4.hide();
        multiPromptUi();
      }) : undefined : undefined), searchBar);
      prompts.forEach((prp) => {
        const i = makePrompt(prp);
        i.appendTo(promptBox);
      });
